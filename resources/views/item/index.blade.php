...
function getData(id){
   $('.modal-footer .btn-primary').show();
           ...
           $('.alert-success').hide();
           $("#add-data").show();
            $('.modal-title').html('<h4><b>Edit Food Items</b></h4>');
            $.ajax({
              type:'GET',
              ...
              if(data.html.on_takeaway==1){$('input[name=on_takeaway]').attr('checked', true);}
               else {
                $('input[name=on_takeaway]').attr('checked', false);
              }
               if(data.html.on_party==1){$('input[name=on_party]').attr('checked', true);}
               else {
                $('input[name=on_party]').attr('checked', false);
              }
